...
global.performanceMetrics = {
  ...
  concurrentUsers: parseInt(
    process.env.PERFORMANCE_TEST_CONCURRENCY ?? '10',
    10,
  ),
  testDuration: parseInt(process.env.PERFORMANCE_TEST_DURATION ?? '30000', 10), // 30 seconds
  rampUpTime: parseInt(process.env.PERFORMANCE_TEST_RAMP_UP ?? '5000', 10), // 5 seconds
};
...
beforeAll(() => {
  ...
  console.log(
    `- Concurrent users: ${global.performanceMetrics.concurrentUsers}`,
  );
...
beforeEach(() => {
  ...
  if (global.performanceMetrics.currentTest) {
    global.performanceMetrics.testResults =
      global.performanceMetrics.testResults.filter(
...
export const performanceUtils = {
  ...
  runConcurrentRequests: async <T>(
    ...
    const makeRequest = async (): Promise<void> => {
      try {
        const { duration } =
          await performanceUtils.measureExecutionTime(requestFn);
    ...
    while (Date.now() - startTime < duration) {
      ...
      if (requestCount < concurrency) {
        const rampUpDelay =
          (global.performanceMetrics.rampUpTime / concurrency) * requestCount;
        setTimeout(() => {
...
process.on('unhandledRejection', (reason, promise) => {
  console.error(
    'Unhandled Rejection in performance test:',
    promise,
    'reason:',
    reason,
  );
